...
SELECT 
  ...
  ROUND(50 + (ABS(RANDOM()) % 1950) * 1.0 / 1000 * 1000, 2) as amount,
  CASE WHEN (ABS(RANDOM()) % 100) < 95 THEN 'completed' ELSE 'pending' END as status
FROM cnt;

-- Generate order_items for each order (1-3 items per order)
INSERT INTO order_items (order_id, product_id, quantity, unit_price)
SELECT 
  o.order_id,
  (ABS(RANDOM()) % 30) + 1 as product_id,
  (ABS(RANDOM()) % 3) + 1 as quantity,
  (SELECT p.unit_price FROM products p WHERE p.product_id = (ABS(RANDOM()) % 30) + 1 LIMIT 1) as unit_price
FROM orders o
